...
export async function GET(req: Request) {
  try {
    ...
    // Totals
    const [totalCalls, bookings, sales, sumAgg] = await Promise.all([
    ...
    const totalDurationMinutes = sumAgg._sum.durationMinutes ?? 0;
    const averageDurationMinutes =
      totalCalls > 0 ? totalDurationMinutes / totalCalls : 0;

    // By-rep counts (avoid groupBy typing edge-cases)
    ...
    // “Booked call ➜ sale” stats
    const bookedCalls = bookings;
    const bookedCallSales = await prisma.callLog.count({
      where: { ...where, appointmentBooked: true, outcome: "Sale" },
    });
