// app/api/shopify/items-by-supplier/route.ts
...
/**
 * Works with API version 2024-07:
 * - variant.price is a Money (scalar) -> no sub-selections
 * - inventoryItem.unitCost is MoneyV2 -> must select fields (amount)
 * - variant.inventoryQuantity is a number (total across locations)
 */
const QUERY = /* GraphQL */ `
    ...
    while (count < limit) {
      ...
      for (const e of edges) {
        ...
        for (const ve of vEdges) {
          ...
          const priceAmount = Number(v?.price ?? 0) || 0; // Money scalar
          const costAmount = Number(v?.inventoryItem?.unitCost?.amount ?? 0) || 0; // MoneyV2.amount
