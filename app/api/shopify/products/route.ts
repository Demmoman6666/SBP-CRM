  ...
  try {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        return NextResponse.json({ error: `Shopify REST search failed: ${res.status} ${txt}` }, { status: 500 });
      }
      ...
      return NextResponse.json({ error: "Shopify product search failed (GraphQL and REST)" }, { status: 500 });
    }
  }
}
