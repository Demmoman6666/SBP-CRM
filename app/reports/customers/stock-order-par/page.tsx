...
// number formatting helpers (UK style)
    ...
    <div ref={rootRef} className="p-6 space-y-6">
      ...
      <div className="mx-auto max-w-5xl rounded-xl border border-slate-200 bg-white/70 shadow-sm p-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-8 gap-3 items-end">
        ...
        <div className="lg:col-span-3 relative">
          ...
          {/* Suggestion list — plain list items (no pink buttons) */}
        ...
        <table className="min-w-full text-sm">
          <thead className="sticky top-0 z-10 bg-white/90 backdrop-blur border-b border-slate-200">
            <tr>
              {([
                ["sku", "SKU", "text-left"],
                ["productName", "Product", "text-left"],
                ["unitsInWindow", "Units (window)", "text-right"],
                ["avgMonthly", "Avg Monthly", "text-right"],
                ["suggestedMonthlyPAR", "Suggested PAR", "text-right"],
                ["agreedPar", "Agreed PAR", "text-right"],
                ["delta", "Δ (Sug - Agr)", "text-right"],
                ["edit", "Edit PAR", "text-right"],
                ["act", "Actions", "text-right"],
              ] as const).map(([key, label, align]) => (
                <th
                  key={key}
                  className={`px-3 py-2 font-medium text-slate-700 ${align} ${
                    key !== "edit" && key !== "act" ? "cursor-pointer select-none" : ""
                  }`}
                  onClick={() => {
                ...
                >
                  <div className="inline-flex items-center gap-1">
                    <span>{label}</span>
                    {sortKey === (key as any) && <span className="text-slate-400">{sortDir === "asc" ? "▲" : "▼"}</span>}
                  </div>
                </th>
              ))}
            </tr>
          ...
          <tbody className="[&>tr:nth-child(odd)]:bg-slate-50/60">
            {rows.map((r: any) => {
              ...
              const deltaClass =
                delta > 0 ? "text-amber-700" : delta < 0 ? "text-emerald-700" : "text-slate-700";
              ...
              return (
                <tr key={r.sku || r.productName} className="border-b border-slate-200 hover:bg-slate-100">
                  ...
                  <td className="px-3 py-2 text-right tabular-nums">{fmt0(r.unitsInWindow)}</td>
                  <td className="px-3 py-2 text-right tabular-nums">{fmt2(r.avgMonthly)}</td>
                  <td className="px-3 py-2 text-right tabular-nums font-medium">{fmt0(r.suggestedMonthlyPAR)}</td>
                  <td className="px-3 py-2 text-right tabular-nums">{r.agreedPar != null ? fmt0(r.agreedPar) : "—"}</td>
                  <td className={`px-3 py-2 text-right tabular-nums ${deltaClass}`}>{fmt0(delta)}</td>
                  <td className="px-3 py-2 text-right">
                    <input
                      className="w-24 border border-slate-300 rounded-md px-2 py-1 text-right tabular-nums"
                      type="number"
                      step="1"
                      min={0}
                      value={editVal}
      ...
      {/* Notes */}
      <div className="mx-auto max-w-5xl text-xs text-slate-600">
        <ul className="list-disc ml-5 space-y-1">
          <li>
            <b>Customer</b> &amp; <b>Brand</b> search are predictive, styled as plain lists (no pink highlights).
          </li>
          <li>
            <b>Suggested PAR</b> = <code>CEIL(AvgMonthly × (1 + Safety%) × Coverage)</code>, rounded to <b>Pack size</b>.
          </li>
        </ul>
      </div>
    </div>
