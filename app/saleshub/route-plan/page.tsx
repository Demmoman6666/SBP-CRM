      ...
      <section className="card">
        ...
        <p className="small muted">
          {cycleStart
            ...
            : "Set your cycle start date below"}
        </p>
      </section>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 12 }}>
          ...
          <div className="field">
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              />
              <button className="btn" onClick={saveCycleStart} disabled={savingCycle}>
                {savingCycle ? "..." : "Set"}
              </button>
            </div>
          </div>
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        textAlign: "center",
                        cursor: "pointer",
                        border: isSelected ? "2px solid var(--pink)" : isThisToday ? "2px solid var(--text)" : "1px solid var(--border)",
                        background: isSelected ? "var(--pink-light)" : isThisToday ? "#f8fafc" : "#fff",
                        fontSize: "0.75rem",
                    ...
                    >
                      {DAY_SHORT[di]}
                      {isThisToday && (
                        <div style={{ fontSize: "0.6rem", color: "var(--pink-dark)", marginTop: 2 }}>Today</div>
                      )}
                    </button>
                  );
                })}
            ...
            <div>
              <h2 style={{ marginBottom: 2 }}>
                ...
                {selectedWeek === currentWeek && selectedDay === todayDay && cycleStart !== null && (
                  <span style={{ marginLeft: 8, padding: "2px 10px", borderRadius: 999, fontSize: "0.75rem", background: "var(--pink)", color: "#fff", fontWeight: 600 }}>
                    Today
                  </span>
                )}
              </h2>
              <p className="small muted">
                {selectedRepName + " - " + (loading ? "Loading..." : customers.length + " stops")}
              </p>
            </div>
            {customers.length > 0 && (
              ...
              <div key={c.id} style={{ border: "1px solid var(--border)", borderRadius: 12, background: "#fff", overflow: "hidden" }}>
                  ...
                  <div style={{ display: "flex", gap: 12, alignItems: "flex-start", flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ minWidth: 0 }}>
                      ...
                      <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
                        ...
                        {c.customerTelephone && (
                          <a href={"tel:" + c.customerTelephone} className="small muted" style={{ textDecoration: "none" }}>
                            {c.customerTelephone}
                          </a>
                        )}
                        {(c.town || c.postCode) && (
                          <span className="small muted">{[c.town, c.postCode].filter(Boolean).join(", ")}</span>
                        )}
                      </div>
                    </div>
                  </div>
                  <div style={{ display: "flex", gap: 6, flexShrink: 0, flexWrap: "wrap" }}>
                    <Link href={"/customers/" + c.id} className="btn" style={{ fontSize: "0.78rem", padding: "5px 10px" }}>
                      Profile
                    </Link>
                    <Link href={"/calls/new?customerId=" + c.id} className="btn" style={{ fontSize: "0.78rem", padding: "5px 10px" }}>
                      Log Call
                    </Link>
                    <button
                      className="primary"
                ...
                {briefCustomerId === c.id && (briefLoading || briefText || briefError) && (
                  <div ref={briefRef} style={{ borderTop: "1px solid var(--border)", padding: "14px", background: "#fafbfc" }}>
                    {briefLoading && (
                      <p className="small muted">Generating pre-call brief...</p>
                    )}
                    {briefError && (
                      <p className="small" style={{ color: "var(--red)" }}>{briefError}</p>
                    )}
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                          ...
                          <div style={{ display: "flex", gap: 6 }}>
                            ...
                            <button
                              className="btn"
                              style={{ fontSize: "0.72rem", padding: "3px 8px" }}
                              onClick={() => { setBriefCustomerId(null); setBriefText(""); }}
                            >
                              Close
                            </button>
                          </div>
