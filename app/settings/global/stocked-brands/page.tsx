...
async function getAll() {
  return prisma.stockedBrand.findMany({
    orderBy: { name: "asc" },
    select: { id: true, name: true, visibleInCallLog: true },
  });
}

export default async function StockedBrandVisibilityPage() {
  ...
  async function save(formData: FormData) {
    ...
    await fetch(`${process.env.NEXT_PUBLIC_BASE_URL || ""}/api/settings/brand-visibility`, {
      ...
      body: JSON.stringify({ type: "stocked", ids }),
      cache: "no-store",
    });
  }
  ...
  return (
    <div className="card grid" style={{ gap: 12 }}>
      ...
      <form action={save} className="grid" style={{ gap: 8 }}>
        ...
        {items.length === 0 && <div className="small muted">No stocked brands yet.</div>}
        <div className="right">
