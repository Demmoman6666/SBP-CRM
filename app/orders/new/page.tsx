...
export default function NewOrderPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        // Minimal fetch; your existing /api/customers/[id] should work.
    ...
    <div className="grid" style={{ gap: 16 }}>
      ...
      <section className="card row" style={{ justifyContent: "space-between", alignItems: "center" }}>
        <div>
          ...
          {!customerId && <div className="small muted">No customer selected.</div>}
        </div>
      ...
      {/* Search (auto-hides after first add, can be toggled back) */}
      {showSearch ? (
        <section className="card">
                    ...
                    <div className="row" style={{ gap: 8, flexWrap: "wrap", marginTop: 6 }}>
                      {p.variants.map((v) => {
                        ...
                        return (
                          ...
                          >
                            {v.title}
                            {v.sku ? ` • ${v.sku}` : ""} • {fmt(priceGross)} inc VAT
                          </button>
                        );
                      })}
                    </div>
