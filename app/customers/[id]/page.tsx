...
export default async function CustomerDetail({ params }: { params: { id: string } }) {
  ...
  async function addNote(formData: FormData) {
    ...
    await fetch(`${process.env.NEXT_PUBLIC_BASE_URL ?? ""}/api/customers/${customer.id}/notes`, {
      method: "POST",
      body: JSON.stringify({ text, staff })
    });
  }
  ...
  async function addVisit(formData: FormData) {
    ...
    const staff = String(formData.get("staff") || "");
    await fetch(`${process.env.NEXT_PUBLIC_BASE_URL ?? ""}/api/customers/${customer.id}/visits`, {
      method: "POST",
      body: JSON.stringify({ date, summary, staff })
    });
  }
      ...
      <div className="card">
        ...
        <div className="grid grid-2" style={{ marginTop: 10 }}>
          ...
          <div>
            <b>Location</b>
            <p className="small">
              {[customer.address1, customer.address2, customer.town, customer.county, customer.postCode].filter(Boolean).join(", ") || "-"}
            </p>
          </div>
          <div>
            <b>Salon</b>
        ...
        <div className="card">
          ...
          {customer.notesLog.length === 0 ? <p className="small">No notes yet.</p> :
            customer.notesLog.map(n => (
              <div key={n.id} className="row" style={{ justifyContent: "space-between", borderBottom: "1px solid #2a2e42", padding: "8px 0" }}>
                <div>
        ...
        <div className="card">
          ...
          {customer.visits.length === 0 ? <p className="small">No visits yet.</p> :
            customer.visits.map(v => (
              <div key={v.id} className="row" style={{ justifyContent: "space-between", borderBottom: "1px solid #2a2e42", padding: "8px 0" }}>
                <div>
